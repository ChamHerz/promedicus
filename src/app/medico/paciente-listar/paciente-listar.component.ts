...
@Component({
  selector: 'app-medico/paciente-listar',
...
export class PacienteListarComponent {
  grid= {
    ...
    botonHistoriaClinica: {col: 1, ro: 1}
  }
  ...
  layoutChanges = this.breakpointObserver.observe(Breakpoints.Handset).subscribe(result => {
    if(result.matches){
      ...
      this.grid = {
        ...
        botonHistoriaClinica: {col: 6, ro: 1}
      }
      return;
    }
    //escritorio
  ...
  botonHistoriaClinica() {
    if (this.selection.selected.length == 0) {
      ...
      let pacienteSeleccionado: PacienteInterface;
      pacienteSeleccionado = this.selection.selected[0];
      this.router.navigate(['medico/paciente/historia', pacienteSeleccionado.dni]);
    }
